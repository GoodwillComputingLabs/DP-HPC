...
# Get information about ALCF system logs in a common folder
folder_path <- "./data/ALCF/Intrepid"
folder_path <- "./data/ALCF/Cooley"
folder_path <- "./data/ALCF/Mira"
folder_path <- "./data/ALCF/Polaris"
folder_path <- "./data/ALCF/Theta"
...
for (csv_file in csv_files) {
    ...
    df %>% group_by(UserID) %>% summarise(n=n()) -> df_unique_users
    cat("Number of distinct users:", nrow(df_unique_users), "\n")

    notna_count <- sum(!is.na(df$UserID))
    cat("Jobs by known users:", notna_count, "\n")

    na_count <- sum(is.na(df$UserID))
